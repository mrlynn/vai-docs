...
## Viewing Models in the Playground

Model information is integrated throughout `vai playground`. When selecting a model in any tab (Embed, Search, Similarity, Rerank), you can see the available models with their key attributes.

### Where Models Appear

**Model dropdowns**: Every tab that calls the Voyage AI API includes a model selector. The dropdown shows model names with their type and pricing.

**Estimate tab**: The cost estimation view lets you compare models side by side, showing per-token pricing and projected costs for your workload.

### Model Information

Each model dropdown provides:
- Model name and type (embedding, reranking, multimodal)
- Pricing per million tokens
- Supported dimensions (for embedding models)
- Context window size

For detailed benchmarks and architecture information, use the CLI with `vai models --benchmarks` or explore topics with `vai explain`.
